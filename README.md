...
- `文本分析 API`：只用于自动化整理的素材分析，不能用于生图。

后端会再次校验 API 用途，即使前端参数被手动修改，也不会把文本分析 API 发送到生图接口。两类配置分别维护默认项。
...
素材分析 API 在“API 设置”中新增为“文本分析 API”。自动化整理页只列出已启用的文本分析配置，分析前可以切换；生图配置不会出现在该下拉框中。
