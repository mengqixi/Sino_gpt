...
素材分析 API 可在页面中保存，也可以在服务器进程中使用环境变量覆盖：

```text
VIP_ANALYSIS_API_KEY=你的密钥
VIP_ANALYSIS_BASE_URL=https://n.tokeness.io/v1
VIP_ANALYSIS_MODEL=gpt-5.6-sol
```

确认后可生成符合唯品会文件名和尺寸要求的图片，并下载ZIP。`401.jpg`由商品资料表单排版，
`606.jpg`由四张来源图组成，`801.jpg`需要上传吊牌图。
